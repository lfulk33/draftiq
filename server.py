...
def api_draft(draft_id):
    try:
        ...
        league_detail = __import__("sleeper_league").get_league(league_id)
        ...
        # Build picks feed with player details
        picks_feed = []
        for p in picks:
            pid = p.get("player_id")
            player = PLAYERS.get(pid, {}) if pid else {}
            picks_feed.append({
...
def api_recommend():
    try:
        ...
        current_pick = len(picks) + 1
        rec = get_recommendation(picks, available, my_roster, league_context, current_pick, PLAYERS)
